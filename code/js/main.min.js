...
$(document).ready(function() {
	...
	function writeStages(){
		...
		var stgIcons = [[],[]];
		for(i=0; i<3; i++){ grids[i].getItems().forEach(function(e){stgIcons[0].push(e.getElement().firstChild.classList[2])}); }
		for(i=3; i<6; i++){ grids[i].getItems().forEach(function(e){stgIcons[1].push(e.getElement().firstChild.classList[2])}); }
		var page1List = "", page2List = "";
		for(i=0;i<stgIcons[0].length;i++){ page1List+=stgIcons[0][i]; }
		for(i=0;i<stgIcons[1].length;i++){ page2List+=stgIcons[1][i]; }
		if(page1Amount%8 != 0){
